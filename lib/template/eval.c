...
static gboolean
_should_render(const gchar *value, LogMessageValueType value_type)
{
  return value_type == LM_VT_BYTES || value_type == LM_VT_PROTOBUF || value[0];
}
  ...
  for (GList *p = self->compiled_template; p; p = g_list_next(p), first_elem = FALSE)
    {
      ...
      switch (e->type)
        ...
        case LTE_VALUE:
        {
          ...
          if (value && _should_render(value, value_type))
            {

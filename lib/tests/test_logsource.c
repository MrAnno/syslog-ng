...
static void
assert_post_messages(LogSource *source, gsize messages_to_send)
{
...
Test(log_source, test_suspend_wakeup)
{
...
Test(log_source, test_forced_suspend)
{
  LogSource *source = test_source_init(&source_options);
...
static void
_try_to_decrase_window(LogSource *source, DynamicWindowPool *pool)
{
...
Test(log_source, test_dynamic_window_reclaim)
{
  ...
  const gsize num_of_pending_messages = pool_size + source_options.init_window_size;
  assert_post_messages(source, num_of_pending_messages);
  cr_assert_not(log_source_free_to_send(source),
                "Source should be suspended, its window is filled with pending messages");

  _try_to_decrase_window(source, pool);
  cr_assert_not(log_source_free_to_send(source));
  ...
  cr_assert_eq(pool->free_window, pool_size,

...
void
log_threaded_dest_driver_insert_batch_length(LogThreadedDestDriver *self, gsize len)
{
  stats_aggregator_insert_data(self->max_batch_size, len);
  stats_aggregator_insert_data(self->average_mbatch_size, len);
}
...
log_threaded_dest_driver_register_aggregated_stats(LogThreadedDestDriver *self)
{
  ...
  stats_register_aggregator_average(0, &sc_key, &self->average_mbatch_size);
...
log_threaded_dest_driver_unregister_aggregated_stats(LogThreadedDestDriver *self)
{
  ...
  stats_unregister_aggregator_average(self->average_mbatch_size);

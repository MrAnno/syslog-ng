...
#define CRT_FILE "/home/anno/Work/syslog-ng/tls/localhost.crt"
#define KEY_FILE "/home/anno/Work/syslog-ng/tls/localhost.key"
...
void send_4080_bytes_in_chunks(SSL *ssl)
{
    const int to_send = 4080;

    const size_t buf_size = 204;
    char buf[buf_size];
    ...
    while (remaining != 0) {
        int w = SSL_write(ssl, buf, remaining < buf_size ? remaining : buf_size);
        if (w <= 0)
            exit(EXIT_FAILURE);

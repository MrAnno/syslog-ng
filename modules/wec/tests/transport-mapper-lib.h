...
#ifndef WEC_OPTIONS_LIB_H_INCLUDED
#define WEC_OPTIONS_LIB_H_INCLUDED
...
assert_transport_mapper_apply(TransportMapper *self, const gchar *transport)
{
  ...
  assert_true(transport_mapper_apply_transport(self, configuration), "wec_apply_transport() failed");
}
...
assert_transport_mapper_apply_fails(TransportMapper *self, const gchar *transport)
{
  if (transport)
    transport_mapper_set_transport(self, transport);
  assert_false(transport_mapper_apply_transport(self, configuration), "wec_apply_transport() succeeded while we expected failure");
}

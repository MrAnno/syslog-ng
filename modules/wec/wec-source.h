...
#define WEC_WNDSIZE_INITED      0x10000
...
void wec_sd_set_keep_alive(LogDriver *self, gint enable);
void wec_sd_set_max_connections(LogDriver *self, gint max_connections);
void wec_sd_set_listen_backlog(LogDriver *self, gint listen_backlog);

static inline gboolean
wec_sd_acquire_socket(AFSocketSourceDriver *s, gint *fd)
{
  if (s->acquire_socket)
...
static inline gboolean
wec_sd_setup_addresses(AFSocketSourceDriver *s)
{
  return s->setup_addresses(s);
}

LogTransport *wec_sd_construct_transport_method(AFSocketSourceDriver *self, gint fd);
gboolean wec_sd_setup_addresses_method(AFSocketSourceDriver *self);

gboolean wec_sd_init_method(LogPipe *s);
gboolean wec_sd_deinit_method(LogPipe *s);
void wec_sd_free_method(LogPipe *self);

void wec_sd_init_instance(AFSocketSourceDriver *self, SocketOptions *socket_options, TransportMapper *transport_mapper, GlobalConfig *cfg);

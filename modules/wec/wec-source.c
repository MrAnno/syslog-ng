...
static void wec_sd_close_connection(AFSocketSourceDriver *self, AFSocketSourceConnection *sc);

static gchar *
wec_sc_stats_instance(AFSocketSourceConnection *self)
{
...
static LogTransport *
wec_sc_construct_transport(AFSocketSourceConnection *self, gint fd)
{
  return transport_mapper_construct_log_transport(self->owner->transport_mapper, fd);
}

static gboolean
wec_sc_init(LogPipe *s)
{
  ...
  if (!self->reader)
    {
      transport = wec_sc_construct_transport(self, self->sock);
...
static void
wec_sc_notify(LogPipe *s, gint notify_code, gpointer user_data)
{
  AFSocketSourceConnection *self = (AFSocketSourceConnection *) s;
  ...
  switch (notify_code)
    ...
    case NC_READ_ERROR:
    {
      if (self->owner->transport_mapper->sock_type == SOCK_STREAM)
        wec_sd_close_connection(self->owner, self);
      break;
    }
    }
}
...
static void
wec_sc_set_owner(AFSocketSourceConnection *self, AFSocketSourceDriver *owner)
{
...
AFSocketSourceConnection *
wec_sc_new(GSockAddr *peer_addr, int fd, GlobalConfig *cfg)
{
  AFSocketSourceConnection *self = g_new0(AFSocketSourceConnection, 1);

  log_pipe_init_instance(&self->super, cfg);
  self->super.init = wec_sc_init;
  self->super.deinit = wec_sc_deinit;
  self->super.notify = wec_sc_notify;
  self->super.free_fn = wec_sc_free;
...
void
wec_sd_add_connection(AFSocketSourceDriver *self, AFSocketSourceConnection *connection)
{
  self->connections = g_list_prepend(self->connections, connection);
}

static void
wec_sd_kill_connection(AFSocketSourceConnection *connection)
{
...
static void
wec_sd_kill_connection_list(GList *list)
{
  GList *l, *next;
  ...
  for (l = list; l; l = next)
    {
      ...
      wec_sd_kill_connection(connection);
    }
}

void
wec_sd_set_keep_alive(LogDriver *s, gint enable)
{
...
void
wec_sd_set_max_connections(LogDriver *s, gint max_connections)
{
  AFSocketSourceDriver *self = (AFSocketSourceDriver *) s;
...
void
wec_sd_set_listen_backlog(LogDriver *s, gint listen_backlog)
{
  AFSocketSourceDriver *self = (AFSocketSourceDriver *) s;
...
static const gchar *
wec_sd_format_name(const LogPipe *s)
{
  ...
  if (s->persist_name)
    {
      g_snprintf(persist_name, sizeof(persist_name), "wec_sd.%s",
  ...
  else
    {
      ...
      g_snprintf(persist_name, sizeof(persist_name), "wec_sd.(%s,%s)",
...
static const gchar *
wec_sd_format_listener_name(const AFSocketSourceDriver *self)
{
  static gchar persist_name[1024];

  g_snprintf(persist_name, sizeof(persist_name), "%s.listen_fd",
             wec_sd_format_name((const LogPipe *)self));

  return persist_name;
}

static const gchar *
wec_sd_format_connections_name(const AFSocketSourceDriver *self)
{
  static gchar persist_name[1024];
...
static gboolean
wec_sd_process_connection(AFSocketSourceDriver *self, GSockAddr *client_addr, GSockAddr *local_addr, gint fd)
{
  ...
  if (self->num_connections >= self->max_connections)
  ...
  else
    {
      ...
      conn = wec_sc_new(client_addr, fd, self->super.super.super.cfg);
      wec_sc_set_owner(conn, self);
      if (log_pipe_init(&conn->super))
        {
          wec_sd_add_connection(self, conn);
  ...
  while (accepts < MAX_ACCEPTS_AT_A_TIME)
    {
      ...
      res = wec_sd_process_connection(self, peer_addr, self->bind_addr, new_fd);
...
static void
wec_sd_close_connection(AFSocketSourceDriver *self, AFSocketSourceConnection *sc)
{
  ...
  wec_sd_kill_connection(sc);
  self->num_connections--;
}

static void
wec_sd_start_watches(AFSocketSourceDriver *self)
{
  IV_FD_INIT(&self->listen_fd);
  self->listen_fd.fd = self->fd;
  self->listen_fd.cookie = self;
  self->listen_fd.handler_in = wec_sd_accept;
  iv_fd_register(&self->listen_fd);
}

static void
wec_sd_stop_watches(AFSocketSourceDriver *self)
{
...
static gboolean
wec_sd_setup_reader_options(AFSocketSourceDriver *self)
{
...
static gboolean
wec_sd_setup_transport(AFSocketSourceDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
...
static gboolean
wec_sd_restore_kept_alive_connections(AFSocketSourceDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
  ...
  if (self->connections_kept_alive_accross_reloads)
    {
      GList *p = NULL;
      self->connections = cfg_persist_config_fetch(cfg, wec_sd_format_connections_name(self));

      self->num_connections = 0;
      for (p = self->connections; p; p = p->next)
        {
          wec_sc_set_owner((AFSocketSourceConnection *) p->data, self);
          if (log_pipe_init((LogPipe *) p->data))
          ...
          else
            {
              ...
              self->connections = g_list_remove(self->connections, sc);
              wec_sd_kill_connection((AFSocketSourceConnection *)sc);
            }
        }
    }
  return TRUE;
...
static gboolean
wec_sd_open_listener(AFSocketSourceDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
  gint sock;
  gboolean res = FALSE;
  ...
  if (self->transport_mapper->sock_type == SOCK_STREAM)
      ...
      if (self->connections_kept_alive_accross_reloads)
        {
          ...
          sock = GPOINTER_TO_UINT(
                   cfg_persist_config_fetch(cfg, wec_sd_format_listener_name(self))) -
                 1;
        }

      if (sock == -1)
        {
          if (!wec_sd_acquire_socket(self, &sock))
            return self->super.super.optional;
          if (sock == -1
              && !transport_mapper_open_socket(self->transport_mapper, self->socket_options, self->bind_addr, WEC_DIR_RECV,
                                               &sock))
            return self->super.super.optional;
        }
  ...
  else
    {
      ...
      if (self->connections || wec_sd_process_connection(self, NULL, self->bind_addr, sock))
        res = TRUE;
    }
  return res;
}
...
static void
wec_sd_close_fd(gpointer value)
{
...
static void
wec_sd_save_connections(AFSocketSourceDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);

  if (!self->connections_kept_alive_accross_reloads || !cfg->persist)
    {
      wec_sd_kill_connection_list(self->connections);
    }
  else
    {
      GList *p;
      ...
      cfg_persist_config_add(cfg, wec_sd_format_connections_name(self), self->connections,
                             (GDestroyNotify)wec_sd_kill_connection_list, FALSE);
    }
  self->connections = NULL;
}

static void
wec_sd_save_listener(AFSocketSourceDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);

  if (self->transport_mapper->sock_type == SOCK_STREAM)
    {
      wec_sd_stop_watches(self);
      if (!self->connections_kept_alive_accross_reloads)
        {
      ...
      else
        {
          ...
          cfg_persist_config_add(cfg, wec_sd_format_listener_name(self),
                                 GUINT_TO_POINTER(self->fd + 1), wec_sd_close_fd, FALSE);
        }
    }
}
...
gboolean
wec_sd_setup_addresses_method(AFSocketSourceDriver *self)
{
  return TRUE;
}

gboolean
wec_sd_init_method(LogPipe *s)
{
  AFSocketSourceDriver *self = (AFSocketSourceDriver *) s;

  return log_src_driver_init_method(s) &&
         wec_sd_setup_transport(self) &&
         wec_sd_setup_addresses(self) &&
         wec_sd_restore_kept_alive_connections(self) &&
         wec_sd_open_listener(self) &&
         transport_mapper_init(self->transport_mapper);
}

gboolean
wec_sd_deinit_method(LogPipe *s)
{
  AFSocketSourceDriver *self = (AFSocketSourceDriver *) s;

  wec_sd_save_connections(self);
  wec_sd_save_listener(self);

  return log_src_driver_deinit_method(s);
}

static void
wec_sd_notify(LogPipe *s, gint notify_code, gpointer user_data)
{
  switch (notify_code)
    {
    case NC_CLOSE:
                          ...
                          GlobalConfig *cfg)
{
  ...
  self->super.super.super.init = wec_sd_init_method;
  self->super.super.super.deinit = wec_sd_deinit_method;
  self->super.super.super.free_fn = wec_sd_free_method;
  self->super.super.super.notify = wec_sd_notify;
  self->super.super.super.generate_persist_name = wec_sd_format_name;
  self->setup_addresses = wec_sd_setup_addresses_method;

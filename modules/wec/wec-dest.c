...
_reload_store_item_new(AFSocketDestDriver *wec_dd)
{
  ReloadStoreItem *item = g_new(ReloadStoreItem, 1);
  item->proto_factory = wec_dd->proto_factory;
  item->writer = wec_dd->writer;
  return item;
}
...
void
wec_dd_set_keep_alive(LogDriver *s, gboolean enable)
{
...
static const gchar *_module_name = "wec_dd";
...
static const gchar *
wec_dd_format_name(const LogPipe *s)
{
...
static const gchar *
wec_dd_format_qfile_name(const AFSocketDestDriver *self)
{
  static gchar persist_name[1024];
...
static const gchar *
wec_dd_format_connections_name(const AFSocketDestDriver *self)
{
  static gchar persist_name[1024];
...
static gchar *
wec_dd_stats_instance(AFSocketDestDriver *self)
{
  static gchar buf[256];

  g_snprintf(buf, sizeof(buf), "%s,%s", self->transport_mapper->transport, wec_dd_get_dest_name(self));
  return buf;
}

static gboolean wec_dd_connected(AFSocketDestDriver *self);
static void wec_dd_reconnect(AFSocketDestDriver *self);
static void wec_dd_try_connect(AFSocketDestDriver *self);
static gboolean wec_dd_setup_connection(AFSocketDestDriver *self);

static void
wec_dd_init_watches(AFSocketDestDriver *self)
{
  IV_FD_INIT(&self->connect_fd);
  self->connect_fd.cookie = self;
  self->connect_fd.handler_out = (void (*)(void *)) wec_dd_connected;
   ...
   * We'll change this to wec_dd_reconnect when the initialization of the
   * connection succeeds.*/
  self->reconnect_timer.handler = (void (*)(void *)) wec_dd_try_connect;
}

static void
wec_dd_start_watches(AFSocketDestDriver *self)
{
...
static void
wec_dd_stop_watches(AFSocketDestDriver *self)
{
  main_loop_assert_main_thread();
...
static void
wec_dd_start_reconnect_timer(AFSocketDestDriver *self)
{
  main_loop_assert_main_thread();
...
static LogTransport *
wec_dd_construct_transport(AFSocketDestDriver *self, gint fd)
{
  return transport_mapper_construct_log_transport(self->transport_mapper, fd);
}

static gboolean
wec_dd_connected(AFSocketDestDriver *self)
{
  ...
  transport = wec_dd_construct_transport(self, self->fd);
  if (!transport)
    goto error_reconnect;
...
error_reconnect:
  close(self->fd);
  self->fd = -1;
  wec_dd_start_reconnect_timer(self);
  return FALSE;
}

static gboolean
wec_dd_start_connect(AFSocketDestDriver *self)
{
  int sock, rc;
  ...
  if (!transport_mapper_open_socket(self->transport_mapper, self->socket_options, self->bind_addr, WEC_DIR_SEND,
                                    &sock))
    {
      return FALSE;
    }
...
static void
wec_dd_reconnect(AFSocketDestDriver *self)
{
  if (!wec_dd_setup_addresses(self) ||
      !wec_dd_start_connect(self))
    {
...
static void
wec_dd_try_connect(AFSocketDestDriver *self)
{
  if (!wec_dd_setup_addresses(self) ||
      !wec_dd_setup_connection(self))
    {
      msg_error("Initiating connection failed, reconnecting",
                evt_tag_int("time_reopen", self->time_reopen));
      wec_dd_start_reconnect_timer(self);
  ...
  self->reconnect_timer.handler = (void (*)(void *)) wec_dd_reconnect;
}

static gboolean
wec_dd_setup_proto_factory(AFSocketDestDriver *self)
{
...
static gboolean
wec_dd_setup_writer_options(AFSocketDestDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
...
static gboolean
wec_dd_setup_transport(AFSocketDestDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
  ...
  if (!wec_dd_setup_proto_factory(self))
    return FALSE;

  if (!wec_dd_setup_writer_options(self))
    return FALSE;
  return TRUE;
}

gboolean
wec_dd_setup_addresses_method(AFSocketDestDriver *self)
{
  return TRUE;
}

static void
wec_dd_restore_writer(AFSocketDestDriver *self)
{
  GlobalConfig *cfg;
  ...
  item = cfg_persist_config_fetch(cfg, wec_dd_format_connections_name(self));
...
LogWriter *
wec_dd_construct_writer_method(AFSocketDestDriver *self)
{
...
static gboolean
wec_dd_setup_writer(AFSocketDestDriver *self)
{
  wec_dd_restore_writer(self);
  ...
  if (!self->writer)
    {
      ...
      self->writer = wec_dd_construct_writer(self);
    }
  ...
  log_writer_set_options(self->writer, &self->super.super.super,
                         &self->writer_options,
                         self->super.super.id,
                         wec_dd_stats_instance(self));
  log_writer_set_queue(self->writer, log_dest_driver_acquire_queue(
                         &self->super, wec_dd_format_qfile_name(self)));
...
static gboolean
wec_dd_setup_connection(AFSocketDestDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
...
gboolean
wec_dd_init(LogPipe *s)
{
  AFSocketDestDriver *self = (AFSocketDestDriver *) s;

  if (!log_dest_driver_init_method(s) ||
      !wec_dd_setup_transport(self))
    {
      return FALSE;
    }
  ...
  if (!wec_dd_setup_writer(self))
    return FALSE;

  wec_dd_try_connect(self);
  return TRUE;
}

static void
wec_dd_stop_writer(AFSocketDestDriver *self)
{
  if (self->writer)
...
static void
wec_dd_save_connection(AFSocketDestDriver *self)
{
  GlobalConfig *cfg = log_pipe_get_config(&self->super.super.super);
  ...
  if (self->connections_kept_alive_accross_reloads)
    {
      ...
      cfg_persist_config_add(cfg, wec_dd_format_connections_name(self), item,
...
gboolean
wec_dd_deinit(LogPipe *s)
{
  AFSocketDestDriver *self = (AFSocketDestDriver *) s;

  wec_dd_stop_watches(self);
  wec_dd_stop_writer(self);

  if (self->connection_initialized)
    {
      wec_dd_save_connection(self);
    }
...
static void
wec_dd_notify(LogPipe *s, gint notify_code, gpointer user_data)
{
  AFSocketDestDriver *self = (AFSocketDestDriver *) s;
...
void
wec_dd_init_instance(AFSocketDestDriver *self,
                          ...
                          GlobalConfig *cfg)
{
  ...
  self->super.super.super.init = wec_dd_init;
  self->super.super.super.deinit = wec_dd_deinit;
  self->super.super.super.free_fn = wec_dd_free;
  self->super.super.super.notify = wec_dd_notify;
  self->super.super.super.generate_persist_name = wec_dd_format_name;
  self->setup_addresses = wec_dd_setup_addresses;
  self->construct_writer = wec_dd_construct_writer_method;

...
static void
_log_py_object_construction_error(void)
{
  gchar buf[256];
  _py_format_exception_text(buf, sizeof(buf));
  msg_error("Error while constructing python object", evt_tag_str("exception", buf));
  _py_finish_exception_handling();
}

static void
_log_py_object_conversion_error_with_exception(void)
{
  gchar buf[256];
  _py_format_exception_text(buf, sizeof(buf));
  msg_error("Error while converting python object", evt_tag_str("exception", buf));
  _py_finish_exception_handling();
}

static void
_log_py_object_conversion_error_with_invalid_type(PyObject *obj, const gchar *expected_type)
{
  msg_error("Error while converting python object",
            evt_tag_str("expected_type", expected_type),
            evt_tag_str("actual_type", Py_TYPE(obj)->tp_name));
}
...
py_bytes_from_string(const char *value, gssize len)
{
  ...
  PyObject *obj = PyBytes_FromStringAndSize(value, len);
  if (!obj)
    _log_py_object_construction_error();
...
{
  PyObject *obj = PyLong_FromLong(l);
  if (!obj)
    _log_py_object_construction_error();
...
{
  PyObject *obj = PyBool_FromLong(b);
  if (!obj)
    _log_py_object_construction_error();
...
py_bytes_or_string_to_string(PyObject *obj, const gchar **string)
{
  if (!is_py_obj_bytes_or_string_type(obj))
    {
      _log_py_object_conversion_error_with_invalid_type(obj, "str/bytes");
      return FALSE;
    }
  ...
  if (!string_local)
    {
      _log_py_object_conversion_error_with_exception();
      return FALSE;
    }
...
py_long_to_long(PyObject *obj, gint64 *l)
{
  if (!PyLong_Check(obj))
    {
      _log_py_object_conversion_error_with_invalid_type(obj, "int");
      return FALSE;
    }
...
py_double_to_double(PyObject *obj, gdouble *d)
{
  if (!PyFloat_Check(obj))
    {
      _log_py_object_conversion_error_with_invalid_type(obj, "float");
      return FALSE;
    }
...
py_boolean_to_boolean(PyObject *obj, gboolean *b)
{
  if (!PyBool_Check(obj))
    {
      _log_py_object_conversion_error_with_invalid_type(obj, "bool");
      return FALSE;
    }
...
py_list_to_list(PyObject *obj, GString *list)
{
  if (!PyList_Check(obj))
    {
      _log_py_object_conversion_error_with_invalid_type(obj, "list");
      return FALSE;
    }

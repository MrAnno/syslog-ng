...
static gboolean
_generate_random_bytes(guint8 *random, guint size, gboolean urandom)
{
  guint length = 0;

  while (length < size)
    {
      gssize rc = getrandom(random + length, size - length, urandom ? 0 : GRND_RANDOM);
...
_run(LogThreadedSourceDriver *s)
{
  ...
  while (!g_atomic_counter_get(&self->exit_requested))
    {
      if (_generate_random_bytes(random_bytes, self->bytes, self->urandom))
        {
...
_format_stats_instance(LogThreadedSourceDriver *s)
{
  ...
  if (s->super.super.super.persist_name)
    ...
    g_snprintf(persist_name, sizeof(persist_name), "random-generator,%u,%u,%d", self->bytes, self->freq, self->urandom);

  return persist_name;
}

LogSourceOptions *
threaded_random_generator_sd_get_source_options(LogDriver *s)
{
  ThreadedRandomGeneratorSourceDriver *self = (ThreadedRandomGeneratorSourceDriver *) s;

  return &self->super.worker_options.super;
}

void
threaded_random_generator_sd_set_freq(LogDriver *s, guint freq)
{
  ThreadedRandomGeneratorSourceDriver *self = (ThreadedRandomGeneratorSourceDriver *) s;
  self->freq = freq;
}

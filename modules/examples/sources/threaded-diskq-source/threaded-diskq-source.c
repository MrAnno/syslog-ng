...
typedef struct ThreadedDiskqFetcher
{
...
} ThreadedDiskqFetcher;

static gboolean
_load_queue(ThreadedDiskqFetcher *self)
{
  ThreadedDiskqSourceDriver *owner = (ThreadedDiskqSourceDriver *) self->super.super.owner;

  self->diskq_options.read_only = TRUE;
  self->diskq_options.reliable = FALSE;

  FILE *file = fopen(owner->filename, "rb");

  if (!file)
    {
      msg_error("Error opening diskq file", evt_tag_str("file", owner->filename));
      return FALSE;
    }
  ...
  if (fread(file_signature, 4, 1, file) == 0)
    {
      msg_error("Error reading diskq file signature", evt_tag_str("file", owner->filename));
      fclose(file);
      return FALSE;
    }
  ...
  if (!log_queue_disk_load_queue(self->queue, owner->filename))
    {
      msg_error("Error loading diskq", evt_tag_str("file", owner->filename));
      return FALSE;
    }
...
static gboolean
_new_diskq_file_exists(ThreadedDiskqFetcher *self, const struct stat *new_diskq_file_stat)
{
...
static gboolean
_open_diskq(LogThreadedFetcher *s)
{
  ThreadedDiskqFetcher *self = (ThreadedDiskqFetcher *) s;
  ThreadedDiskqSourceDriver *owner = (ThreadedDiskqSourceDriver *) self->super.super.owner;

  struct stat new_diskq_file_stat;
  if (stat(owner->filename, &new_diskq_file_stat) != 0)
    {
      msg_info("Diskq file does now exist, retrying", evt_tag_str("file", owner->filename));
      return FALSE;
    }
  ...
  if (self->waiting_for_file_change)
    {
      if(!_new_diskq_file_exists(self, &new_diskq_file_stat))
        {
          msg_debug("Still waiting for new file", evt_tag_str("file", owner->filename));
          return FALSE;
        }
...
static void
_close_diskq(LogThreadedFetcher *s)
{
  ThreadedDiskqFetcher *self = (ThreadedDiskqFetcher *) s;
...
{
  ThreadedDiskqFetcher *self = (ThreadedDiskqFetcher *) s;
  ThreadedDiskqSourceDriver *owner = (ThreadedDiskqSourceDriver *) s->super.owner;
  ...
  if (!msg)
    {
      if (remaining_messages != 0)
        msg_error("Closing corrupt diskq file, waiting for new", evt_tag_long("lost_messages", remaining_messages),
                  evt_tag_str("file", owner->filename));
      else
        msg_info("Diskq file has been read, waiting for new file", evt_tag_str("file", owner->filename));
...
_fetcher_new(LogThreadedSourceDriver *drv)
{
  GlobalConfig *cfg = log_pipe_get_config(&drv->super.super.super);

  ThreadedDiskqFetcher *self = g_new0(ThreadedDiskqFetcher, 1);
...
threaded_diskq_sd_new(GlobalConfig *cfg)
{
  ...
  self->super.construct_worker = _fetcher_new;

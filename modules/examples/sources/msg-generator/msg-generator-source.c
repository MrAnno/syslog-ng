  ...
  if (self->options->template)
    {
      ...
      log_template_format(self->options->template, msg, NULL, LTZ_LOCAL, self->seq_num, NULL, msg_body);
...
void
msg_generator_source_set_options(MsgGeneratorSource *self, LogPipe *control, MsgGeneratorSourceOptions *options,
                                 ...
                                 gboolean pos_tracked, LogExprNode *expr_node)
{
  ...
  /* self->control is unused, but its lifetime is extended this way by AckTracker */
  log_pipe_unref(self->control);
